...
ROAD_LENGTH, ROAD_WIDTH, X_PIXEL, Y_PIXEL = 1640, 50, 1366, 90
...
def create_frame_list(DATASET_DIRECTORY):
    tracks = csv.DictReader(open(DATASET_DIRECTORY + "us_101_trajectories.csv"))

    frame_list = []
    frame_IDs = tracks['frame_ID']

    frame_len = max(frame_IDs) #int(Decimal(recordingMeta['duration']) * int(recordingMeta['frameRate']))
    for i in range(frame_len):
        frame_list.append([])

    print('Loading frame list ...')
    for row in tracks:
        frame_list[int(row['Frame_ID']) - 1].append(row)
...
def get_car_list(frame_list, frame):
    ...
    for row in frame_list[frame]:
        ...
        v_id = row['Vehicle_ID']
        a = float(row['Local_Y']) * X_PIXEL / ROAD_LENGTH
        b = (float(row['Local_X'])+0.2) * Y_PIXEL / ROAD_WIDTH
        w = float(row['v_length']) * X_PIXEL / ROAD_LENGTH
        h = float(row['v_Width']) * Y_PIXEL / ROAD_WIDTH
        ...
        lane = get_lane(b, self.LANES_Y)
        vehicles_id.append(row['Vehicle_ID'])
...
class data_extractor():
    def __init__(self, directory=''):
        self.dataset_directory = directory + "us_101_trajectories.csv"
        self.ROAD_LENGTH = 1640
        self.ROAD_WIDTH = 50
        self.X_PIXEL = 1366
        self.Y_PIXEL = 90
        self.LANES_Y = []
    ...
    def load_data(self):
        ...
        # self.tracks = csv.DictReader(open(self.dataset_directory))
        self.tracks_df = pd.read_csv(self.dataset_directory)
        self.tracks_dict = self.csv_to_dict()
        # self.tracksMeta = pd.read_csv(self.dataset_directory + "tracksMeta.csv")
    
    def csv_to_dict(self):
        dict = []
        with open(self.dataset_directory, 'r') as csvfile:
    ...
    def create_frame_list(self):
        ...
        frame_list = []
        frames = []
        for row in self.tracks_dict:
            frames.append(int(row['Frame_ID']))
        frame_len = max(frames)  
        for i in range(frame_len):
            frame_list.append([])

        # filling the blanks of frame list
        for row in self.tracks_dict:
            if int(row['Vehicle_ID']) % 2 == 0:
                frame_list[int(row['Frame_ID']) - 1].append(row)
    ...
    def get_lanes(self, frame):
        vehicles_lane = []
        for row in self.frame_list[frame]:
            b = (float(row['Local_X'])+0.2) * self.Y_PIXEL / self.ROAD_WIDTH                
            lane = get_lane(b, self.LANES_Y)
            vehicles_lane.append(lane)
    ...
    def get_positions(self, frame):
        vehicles_pos = []
        for row in self.frame_list[frame]:
            a = float(row['Local_Y']) * self.X_PIXEL / self.ROAD_LENGTH
            b = (float(row['Local_X'])+0.2) * self.Y_PIXEL / self.ROAD_WIDTH
            w = float(row['v_length']) * self.X_PIXEL / self.ROAD_LENGTH
            h = float(row['v_Width']) * self.Y_PIXEL / self.ROAD_WIDTH
            vehicles_pos.append((a, b, w, h)) 
        return vehicles_pos
    
    def get_position(self, vId, frame):
        for row in self.frame_list[frame]:
            if int(row['Vehicle_ID']) == int(vId):
                break
        a = float(row['Local_Y']) * self.X_PIXEL / self.ROAD_LENGTH
        b = (float(row['Local_X'])+0.2) * self.Y_PIXEL / self.ROAD_WIDTH
        w = float(row['v_length']) * self.X_PIXEL / self.ROAD_LENGTH
    ...
    def get_velocities(self, frame):
        vehicles_vel = []
        for row in self.frame_list[frame]:
            vehicles_vel.append((float(row['v_Vel']) * self.X_PIXEL / self.ROAD_LENGTH,0))
        return vehicles_vel
        
    def get_velocity(self, vId, frame):
        for row in self.frame_list[frame]:
            if int(row['Vehicle_ID']) == vId:
                break
        return float(row['v_Vel']) * self.X_PIXEL / self.ROAD_LENGTH
        
    def get_accelerations(self, frame):
        vehicles_acc = []
        for row in self.frame_list[frame]:
            vehicles_acc.append(float(row['v_Acc']) * self.X_PIXEL / self.ROAD_LENGTH)
        return vehicles_acc
    ...
    def get_acceleration(self, vId, frame):
        for row in self.frame_list[frame]:
            if int(row['Vehicle_ID']) == vId:
                break
        return float(row['v_Acc']) * self.X_PIXEL / self.ROAD_LENGTH
    
    def get_directions(self, frame):
        vehicles_direction = []
        lanes = self.get_lanes(frame)
        for lane in lanes:
            if 6 <= lane <= 10:  # right-to-left direction
                vehicles_direction.append(-1)
            elif 1 <= lane <= 5:  # left-to-right direction
                vehicles_direction.append(1)
            else:
                vehicles_direction.append(0)
    ...
    def get_action_validity(self, vId,  frame):
        for row in self.frame_list[frame]:
            if int(row['Vehicle_ID']) == int(vId):
                break
        y = (float(row['Local_X'])+0.2)*self.Y_PIXEL/self.ROAD_LENGTH
        lane = get_lane(y, self.LANES_Y)
