...
import pygame
from utils import get_lane, distance, get_most_recent_file
import random
import torch
...
class AgentCar:
    ...
    """
    def __init__(self, width=15, height=8, direction=1, LANES_Y=[]):
    ...
    def reset_episode(self):
        ...
        if self.direction == 1:
            ...
            self.y = self.LANES_Y[self.lane] - semi_lane_width
        else:
            self.lane = random.randint(6, 10)
            self.y = self.LANES_Y[self.lane] - semi_lane_width            

        # reset states
        self.velocity_x = random.randint(30, 50) #self.initial_speed()
        self.V_x_desired = random.randint(30, 50) #self.initial_speed()
        self.velocity_y = 0
    ...
    def init_lane(self, target_vehicles_list):
        ...
        if self.direction == 1:
            ...
            self.y = self.LANES_Y[lane] - semi_lane_width
        ...
        if lane != 0:
            if self.direction == 1:
                ...
                if action == 'right_lane_change':
                    y_desired = self.LANES_Y[lane + 1] - semi_lane_width

                elif action == 'left_lane_change':
                    y_desired = self.LANES_Y[lane - 1] - semi_lane_width

                else:
                    y_desired = self.LANES_Y[lane] - semi_lane_width
    ...
    def get_offroad_reward(self):
        ...
        if self.direction == 1:
            Border1, Border2 = self.LANES_Y[0], self.LANES_Y[-2]
            ...
            Border1, Border2 = self.LANES_Y[0], self.LANES_Y[3]
            if Border1 <= self.y <= Border2:
                reward_out = 0
    ...
    def update_lane(self):
        self.lane = get_lane(self.y, self.LANES_Y)
    ...
    def Degree_of_Safety(self, action, target_vehicles):
        ...
        next_lane_DoS = 1
        next_lane_ego = get_lane(y_next_ego, self.LANES_Y)
