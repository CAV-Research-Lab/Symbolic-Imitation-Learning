...
import numpy as np
import time
from utils import get_distance, get_lane, get_most_recent_file
...
OUTPUT_EXCEL_FILE = 'excel/sil_ngsim_data.xlsx'
...
if __name__ == '__main__':
    ...
    road_length, road_width, X_pixel, Y_pixel = 1366, 50, 1366, 70
    X, Y = 1366, 90
    timestep = 1 / 25
    # ==================================================================
    RADAR_RANGE = 60 # PIXEL
    a, b = 4, 16
    LANES_Y = [a+i*b for i in range(7)]
    timestr = time.strftime("%Y%m%d-%H%M%S")

    # ===============================================================
    # initialize the AgentCar object
    agent = AgentCar(width=15, height=8, direction=DIRECTION, LANES_Y=LANES_Y)
    ...
    extractor.ROAD_LENGTH = road_length
    extractor.ROAD_WIDTH = road_width
    extractor.X_PIXEL = X_pixel
    extractor.Y_PIXEL = Y_pixel
    extractor.LANES_Y = LANES_Y
    ...
    episode = 0
    best_score = -math.inf
    max_distance = N_EPOCHS * road_length
